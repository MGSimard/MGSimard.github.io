...
async function summonerDataHandler() {
    ...
    .then (data => {
        ...
        //WRITE SUMMONER DATA
        sumAvatar.src = "http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/img/profileicon/" + profileIconId + ".png";
...
async function buildMatches(targetPuuid, regionCluster, ddVer) {
    ...
    const allChamps = await fetch("http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/champion.json").then(response => response.json()).then(data => data.data);
    const sumSpells = await fetch("http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/summoner.json").then(response => response.json()).then(data => data.data);
    const runesReforged = await fetch("http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/runesReforged.json").then(response => response.json());
    const itemList = await fetch("http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/item.json").then(response => response.json()).then(data => data.data);
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        //CHAMPION FRAME
        statChampion.setAttribute("style", "background-image: linear-gradient(to right, transparent, var(--lolBlue6)), url('http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/img/champion/" + Object.entries(allChamps).find(([champ, info]) => info.key == targetPlayerData.championId)[1].image.full);
        ...
        //SUMMONER SPELLS
        statSumSpell1.setAttribute("src","http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/img/spell/" + Object.entries(sumSpells).find(([spell, info]) => info.key == targetPlayerData.summoner1Id)[1].image.full);
        statSumSpell2.setAttribute("src","http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/img/spell/" + Object.entries(sumSpells).find(([spell, info]) => info.key == targetPlayerData.summoner2Id)[1].image.full);
        ...
        for (let h = 0; h < 7; h++) {
            ...
            if (itemPath) {
                statItems[h].setAttribute("style","background-image: url(http://ddragon.leagueoflegends.com/cdn/" + ddVer + "/img/item/" + itemList[itemPath].image.full+")");
            }
        }
