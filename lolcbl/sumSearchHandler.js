...
async function summonerDataHandler() {
  ...
  if (existingMatches.innerHTML) {
    existingMatches.innerHTML = "";
  }

  //GET LATEST DATADRAGON VERSION
  const ddVer = await fetch("https://ddragon.leagueoflegends.com/api/versions.json")
  ...
  fetch(
    ...
    .then((data) => {
      ...
      //FETCH LEAGUEV4 BY-SUMMONER (RANKED DATA)
...
async function buildMatches(targetPuuid, regionCluster, ddVer) {
  ...
  //GET DATA LISTS
  const modeList = await fetch("https://static.developer.riotgames.com/docs/lol/queues.json").then((response) =>
    response.json()
  );
  const allChamps = await fetch("https://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/champion.json")
    .then((response) => response.json())
    .then((data) => data.data);
  const sumSpells = await fetch("https://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/summoner.json")
    .then((response) => response.json())
    .then((data) => data.data);
  const runesReforged = await fetch(
    "https://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/runesReforged.json"
  ).then((response) => response.json());
  const itemList = await fetch("https://ddragon.leagueoflegends.com/cdn/" + ddVer + "/data/en_US/item.json")
    .then((response) => response.json())
    .then((data) => data.data);
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const matchInfo = await fetch(
      "https://" + regionCluster + ".api.riotgames.com/lol/match/v5/matches/" + matchId + keyURL
    ).then((response) => response.json());
    const matchType = modeList.find((gameTypes) => gameTypes.queueId == matchInfo.info.queueId);
    const matchDuration =
      Math.floor(matchInfo.info.gameDuration / 60) + "m" + Math.floor(matchInfo.info.gameDuration % 60) + "s";
    ...
    const playerKDA = playerDeaths
      ? ((playerKills + playerAssists) / playerDeaths).toFixed(2)
      : playerKills + playerAssists;
    ...
    //CHAMPION FRAME
    statChampion.setAttribute(
      "style",
      "background-image: linear-gradient(to right, transparent, var(--lolBlue6)), url('https://ddragon.leagueoflegends.com/cdn/" +
        ddVer +
        "/img/champion/" +
        Object.entries(allChamps).find(([champ, info]) => info.key == targetPlayerData.championId)[1].image.full
    );
    ...
    switch (matchType.queueId) {
      case 420:
        ...
        break;
      case 400:
        playedMode = "DRAFT PICK";
        break;
      case 430:
        playedMode = "BLIND PICK";
        break;
      case 440:
        playedMode = "RANKED FLEX";
        break;
      case 450:
        playedMode = "ARAM";
        break;
      case 700:
        ...
        break;
      case 720:
        playedMode = "ARAM CLASH";
        break;
      case 820:
      case 830:
      case 840:
      case 850:
        playedMode = "Co-op vs. AI";
        break;
      case 900:
        playedMode = "ARURF";
        break;
      case 1020:
        playedMode = "ONE FOR ALL";
        break;
      case 1300:
        playedMode = "NEXUS BLITZ";
        break;
      case 1400:
        playedMode = "ULTIMATE SPELLBOOK";
        break;
      case 1900:
      case 76:
        playedMode = "URF";
      ...
      case 75:
        ...
        break;
      default:
        playedMode = matchType.description;
    }
    ...
    let output;
    if (timeSinceEnd < 60) {
      output = timeSinceEnd + " seconds ago";
    } else if (timeSinceEnd < 120) {
      output = Math.floor(timeSinceEnd / 60) + " minute ago";
    } else if (timeSinceEnd < 3600) {
      output = Math.floor(timeSinceEnd / 60) + " minutes ago";
    } else if (timeSinceEnd < 7200) {
      output = Math.floor(timeSinceEnd / 3600) + " hour ago";
    } else if (timeSinceEnd < 86400) {
      output = Math.floor(timeSinceEnd / 3600) + " hours ago";
    } else if (timeSinceEnd < 172800) {
      output = Math.floor(timeSinceEnd / 86400) + " day ago";
    } else if (timeSinceEnd < 604800) {
      output = Math.floor(timeSinceEnd / 86400) + " days ago";
    } else if (timeSinceEnd < 1209600) {
      output = Math.floor(timeSinceEnd / 604800) + " week ago";
    } else if (timeSinceEnd < 2628000) {
      output = Math.floor(timeSinceEnd / 604800) + " weeks ago";
    } else if (timeSinceEnd < 5256000) {
      output = Math.floor(timeSinceEnd / 2620800) + " month ago";
    } else if (timeSinceEnd < 31536000) {
      output = Math.floor(timeSinceEnd / 2620800) + " months ago";
    } //Not going past months, years is too large for accuracy's sake
    ...
    //SUMMONER SPELLS
    statSumSpell1.setAttribute(
      "src",
      "https://ddragon.leagueoflegends.com/cdn/" +
        ddVer +
        "/img/spell/" +
        Object.entries(sumSpells).find(([spell, info]) => info.key == targetPlayerData.summoner1Id)[1].image.full
    );
    statSumSpell2.setAttribute(
      "src",
      "https://ddragon.leagueoflegends.com/cdn/" +
        ddVer +
        "/img/spell/" +
        Object.entries(sumSpells).find(([spell, info]) => info.key == targetPlayerData.summoner2Id)[1].image.full
    );

    //RUNES
    let selectedKeystone;
    for (let k = 0; k < runesReforged.length; k++) {
      const runeTest = runesReforged[k].slots[0].runes.find(
        (keystones) => keystones.id == targetPlayerData.perks.styles[0].selections[0].perk
      );
      if (runeTest) {
    ...
    for (let h = 0; h < 7; h++) {
      ...
      if (itemPath) {
        statItems[h].setAttribute(
          "style",
          "background-image: url(https://ddragon.leagueoflegends.com/cdn/" +
            ddVer +
            "/img/item/" +
            itemList[itemPath].image.full +
            ")"
        );
      }
    }
